//===========================================================================
// feature_map_12_12_syn.v == 用户原件 rtl/pe10_10/feature_map_12_12.v 的**综合用副本**
//   唯一差别：给 feature_map[] 加了 (* syn_ramstyle = "registers" *)
//   目的：避免 EFX-0657 把它 bit-blast 成 logic memory（工具随后在数据库里崩）
//   仿真仍用用户原件；综合工程 XML 里换成这一份。模块名保持一致。
//===========================================================================
module feature_map_12_12(
    input  wire          clk,
    input  wire          rstn,
    input  wire  [17:0]  wdata    [0:143],
    input  wire          wdata_en,
    input  wire          op,
    input  wire          start,   //复用数据卷积首位开始信号,只能9周期拉高一次，不复用数据该信号拉低
    
    output wire  [17:0]  right_a_in_last_line[0:9],
    output wire  [17:0]  buttom_a_in_last_line[0:9],//a为数据
    output wire  [17:0]  load_a_in[0:99],
